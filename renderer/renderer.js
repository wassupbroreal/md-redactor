...
window.api = {
  ...
  saveFile:       (path, content) => invoke && invoke('save_file', { pathStr: path, content: content }),
...
document.addEventListener('keydown', (e) => {
  if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
    e.preventDefault();
    if (el.editorContainer.style.display !== 'none') {
      el.btnSave.click();
    }
  }
});
...
el.btnNew.addEventListener('click', () => {
  ...
  el.editor.value = '';
  el.mdOut.innerHTML = '';
  el.winTitle.textContent = 'Новый файл';
});
...
el.btnSave.addEventListener('click', async () => {
  if (el.editorContainer.style.display === 'none') return;
  
  if (currentFilePath) {
    await window.api.saveFile(currentFilePath, el.editor.value);
    // show some feedback?
    el.winTitle.textContent = currentFilePath + ' (Сохранено)';
    setTimeout(() => { el.winTitle.textContent = currentFilePath; }, 2000);
  } else {
    const path = await window.api.saveDialog();
    if (path) {
      await window.api.saveFile(path, el.editor.value);
      currentFilePath = path;
      el.winTitle.textContent = path;
    }
  }
});

// ── Editor Realtime parsing ───────────────────────────────────────────────────
let parseTimeout = null;
el.editor.addEventListener('input', () => {
  clearTimeout(parseTimeout);
  parseTimeout = setTimeout(() => {
    el.mdOut.innerHTML = parseMarkdown(el.editor.value);
    applyCopyButtons();
  }, 300); // 300ms debounce
});

// ── Spinner / Welcome fallback ──────────────────────────────────────────────────
...
function renderContent(data) {
  ...
  el.mdOut.innerHTML = data.html || parseMarkdown(data.content || '');
  applyCopyButtons();
...
function applySettings() {
  ...
  // Buttons text translation
  $('btn-new').textContent = dict.newBtn;
  $('btn-open').textContent = dict.openBtn;
  $('btn-save').textContent = dict.saveBtn;
