...
#[tauri::command]
fn save_file(path_str: String, content: String) -> Result<(), String> {
...
fn run_uninstall(app: AppHandle) {
    ...
    let reg_commands = r#"
        ...
        reg delete "HKCU\Software\Classes\MDRedactor.Assoc" /f
        reg delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\MDRedactor" /f
    "#;
    ...
    let remove_shortcuts_script = r#"
        $DesktopPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), 'MD Redactor.lnk')
        if (Test-Path $DesktopPath) { Remove-Item $DesktopPath }
        $ProgramsPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Programs'), 'MD Redactor.lnk')
    ...
    if let Ok(install_dir) = app.path().local_data_dir().map(|p| p.join("Programs").join("MD Redactor")) {
        let temp_dir = std::env::temp_dir();
        let batch_path = temp_dir.join("md_redactor_uninstall.bat");
